...
class KerasNetAaf(CnnAafBase):
    ...
    def __init__(
        ...
        self.fc7 = torch.nn.Linear(
            ...
            out_features=self._fc7_out_features, bias=True
        )

        # act7 initialization postponed.

        self.drop7 = torch.nn.Dropout(p=0.2)

        self.fc8 = torch.nn.Linear(
            in_features=self._fc7_out_features, out_features=10, bias=True
        )

        # softmax is embedded in pytorch's loss function

        # Initialize act7 here as it may use the random number generator for
        # init. Using the RNG changes it state. To keep the comparison fair and
        # ensure that all the other params are the same - init act3 after
        # initializing all the other random parameters.
        self.act7 = af_build(self._af_def_fc, in_dims=(self._fc7_out_features,))
